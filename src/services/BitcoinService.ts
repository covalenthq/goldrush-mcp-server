...
export function addBitcoinServiceTools(
    ...
    server.tool(
        ...
        {
            walletAddress: z.string().describe("The xpub key of the Bitcoin HD wallet to get balances for. Must be a valid extended public key."),
            quoteCurrency: z
                .enum(Object.values(validQuoteValues) as [string, ...string[]])
                .optional()
                .describe("Currency to quote Bitcoin values in (e.g., 'USD', 'EUR'). If not specified, uses default quote currency."),
        },
        async (params) => {
            try {
                const response =
    ...
    server.tool(
        ...
        {
            address: z.string().describe("The Bitcoin address to get transaction history for. Must be a valid Bitcoin address."),
            pageSize: z.number().optional().default(10).describe("Number of transactions to return per page. Default is 10, maximum is 100."),
    ...
    server.tool(
        ...
        {
            walletAddress: z.string().describe("The Bitcoin address to get balance for. Must be a valid non-HD Bitcoin address."),
            quoteCurrency: z
                .enum(Object.values(validQuoteValues) as [string, ...string[]])
                .optional()
                .describe("Currency to quote Bitcoin values in (e.g., 'USD', 'EUR'). If not specified, uses default quote currency."),
