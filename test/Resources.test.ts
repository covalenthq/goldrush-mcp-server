...
describe.skipIf(!HAS_GOLDRUSH_API_KEY)("Resources", () => {
    let client: Client;
    ...
    it("Should read status://chain/eth-mainnet resource successfully", async () => {
        ...
        const resp = await client.readResource({
            uri: "status://chain/eth-mainnet",
        });
        expect(resp.contents).toBeDefined();
        expect(resp.contents?.length).toBeGreaterThan(0);
        console.log(
            "status://chain/eth-mainnet resource contents:",
            resp.contents
        );
    }, 30000);
    ...
    it("Should handle an invalid chain", async () => {
        const resp = await client.readResource({
            uri: "status://chain/not-a-chain",
        });
        expect(resp.contents).toBeDefined();
        expect(resp.contents?.length).toBeGreaterThan(0);
        // Potentially it returns an error JSON
        console.log(
            "status://chain/not-a-chain resource contents:",
            resp.contents
        );
    }, 30000);
    ...
    it("Should read config://supported-chains resource successfully", async () => {
        ...
        console.log(
            "config://supported-chains resource contents:",
    ...
    it("Should read config://quote-currencies resource successfully", async () => {
        ...
        console.log(
            "config://quote-currencies resource contents:",
            quoteCurrencies
        );
    }, 30000);
});
